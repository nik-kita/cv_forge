...
import {get_refresh_token} from '@/service/local_storage/persistent.tokens'
import {fn_to_promise_logic} from '@/common_xstate/utils/fn_to_promise_logic'
import {
  assertEvent,
  fromPromise,
  sendParent,
...
export const fetch_machine = setup({
  ...
  guards: {
    ...
    is_error_access_expired({event}) {
      ...
      return [401, 403].includes(
        event.error.err_container[0].status,
      )
    },
  },
}).createMachine({
  ...
  states: {
    ...
    Fail: {
      type: 'final',
      entry: ['fail', 'emit_im_done'],
    },
    Success: {
      type: 'final',
      entry: ['success', 'emit_im_done'],
    },
    Waiting_for_refresh: {
      on: {
